...
// makes sure TokenType is as expected, if not, throws an error
...
static void emitByte(uint8_t byte) { appendChunk(currentChunk(), byte, parser.previous.line); }
...
/* int line = -1;
	for (;;) {
		...
		printf("%d %.*s", token.type, token.length, token.start);
	} */
